...
<div class="row text-center justify-content-center">
    ...
    <div class="col-4 mt-2">
        <div class="card h-100">
            <div class="card-header font-weight-bold">{{$order->item_nombre}}</div>
            <div class="card-body">
                ...
                <h4 class="card-text">${{$order->item_subtotal}}</h4>
                ...
                @switch($order->estado->id)
                    @case(App\Estado::ESTADO_PENDIENTE)
                        <form action="/orders/{{$order->id}}/status" method="POST"> @method('PATCH') @csrf  
                            ...
                            <button type="submit" name="status" value="{{App\Estado::ESTADO_AYUDA}}" class="btn btn-info">Necesito ayuda</button>
                        </form>
                        @break
